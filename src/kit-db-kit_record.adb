...
with Kit.Db.Kit_Record_Cache;

package body Kit.Db.Kit_Record is

   type Kit_Record_Implementation is
        limited new Ada.Finalization.Limited_Controlled
     and Kit_Record_Interface with
      record
         Finished        : Boolean;
         Forward         : Boolean;
         Read_Only       : Boolean;
         ...
         M_Index         : Kit_Record_Reference;
         T1_Data         :
            Kit_Root_Record_Impl.Kit_Root_Record_Database_Record;
         T2_Data         : Kit_Record_Impl.Kit_Record_Database_Record;
      end record;
   overriding function Has_Element (Item : Kit_Record_Implementation)
      return Boolean;
   overriding procedure X_Lock (Item : Kit_Record_Implementation);
   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Record_Implementation) return Kit_Root_Record_Reference;
   overriding function Get_Kit_Record_Reference (Item :
      Kit_Record_Implementation) return Kit_Record_Reference;
   overriding function Top_Record (Item : Kit_Record_Implementation)
      return Record_Type;
   overriding function Name (Item : Kit_Record_Implementation) return String;
   overriding function Table_Index (Item : Kit_Record_Implementation)
      return Integer;
   overriding function Record_Length (Item : Kit_Record_Implementation)
      return Integer;
   overriding function Identity (Item : Kit_Record_Implementation)
      return String;
   overriding function Get
     (Item  : Kit_Record_Implementation;
      Field : String)
   return String;
   ...
   overriding function Previous
     (Object   : Iterator;
      Position : Cursor)
   return Cursor;
   type Kit_Record_Table_Notify_Record is
        new Kit.Notifier.Table_Notify_Interface with
      record
         Handler : Kit_Record_Table_Notify_Handler;
      end record;
   overriding procedure Notify_Table_Change (Handle :
      Kit_Record_Table_Notify_Record);
   type Kit_Record_Notify_Record is
        new Kit.Notifier.Record_Notify_Interface with
      record
         Handler : Kit_Record_Notify_Handler;
      end record;
   overriding procedure Notify_Record_Change
     (Handle         : Kit_Record_Notify_Record;
      Changed_Record : Marlowe.Database_Index);
   type Kit_Record_Update_Implementation is
        limited new Kit_Record_Implementation
     and Kit_Record_Update_Interface
     and Record_Update_Interface with null record;
   overriding procedure Initialize (Item :
      in out Kit_Record_Update_Implementation);
   overriding procedure Finalize (Item :
      in out Kit_Record_Update_Implementation);
   overriding procedure Delete (Item :
      in out Kit_Record_Update_Implementation);
   overriding procedure Set
     (Item  : in out Kit_Record_Update_Implementation;
      Field : String;
      Value : String);
   overriding procedure Set_Name
     (Item  : in out Kit_Record_Update_Implementation;
      Value : String);
   overriding procedure Set_Table_Index
     (Item  : in out Kit_Record_Update_Implementation;
      Value : Integer);
   overriding procedure Set_Record_Length
     (Item  : in out Kit_Record_Update_Implementation;
      Value : Integer);
   ...
   is
      Result : Kit_Record_Update := Create;
   begin
      Result.Set_Name (Name);
      Result.Set_Table_Index (Table_Index);
      Result.Set_Record_Length (Record_Length);
      return Result.Get_Kit_Record_Reference;
   end Create;
   ...
   function Create return Kit_Record_Update is
      T1_Access : Kit_Root_Record_Cache.Cache_Access;
      T2_Access : Kit_Record_Cache.Cache_Access;
   begin
      return Result : Kit_Record_Update_Implementation do
         ...
         T1_Access.Db.Top_Record := R_Kit_Record;
         T2_Access := new Kit_Record_Cache.Cache_Record;
         Memory_Mutex.Unlock;
         Kit_Root_Record_Impl.File_Mutex.Lock;
         T2_Access.Db.Kit_Root_Record := Kit_Root_Record_Reference
            (Marlowe_Keys.Handle.Insert_Record (1));
         T1_Access.Initialise (1, Marlowe.Database_Index
            (T2_Access.Db.Kit_Root_Record));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T1_Access));
         Kit_Root_Record_Impl.Write (Marlowe.Database_Index
            (T2_Access.Db.Kit_Root_Record), T1_Access.Db);
         T1_Access.X_Lock;
         Kit_Root_Record_Impl.File_Mutex.Unlock;
         Kit_Record_Impl.File_Mutex.Lock;
         Result.M_Index := Kit_Record_Reference
            (Marlowe_Keys.Handle.Insert_Record (2));
         T2_Access.Initialise (2, Marlowe.Database_Index (Result.M_Index));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T2_Access));
         Kit_Record_Impl.Write (Marlowe.Database_Index (Result.M_Index),
            T2_Access.Db);
         T2_Access.X_Lock;
         Kit_Record_Impl.File_Mutex.Unlock;
         Result.T1_Data := T1_Access.Db;
         Result.T2_Data := T2_Access.Db;
         Result.Read_Only := False;
      end return;
   end Create;
   ...
   overriding procedure Delete (Item :
      in out Kit_Record_Update_Implementation) is
   begin
   ...
   function Element (Item : Cursor) return Kit_Record_Reference is
   begin
   ...
   begin
      ...
      if Item.Deleted then
         ...
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T2_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T2_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T2_Name_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T2_Table_Index_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Table_Index, 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Item.T1_Data.Deleted := True;
         Item.T2_Data.Deleted := True;
         Kit.Notifier.Record_Deleted (1, Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record));
         Kit.Notifier.Record_Deleted (2, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not (Item.Read_Only or else Item.Created) or else Item.Deleted then
         Kit.Notifier.Record_Changed (1, Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record));
         Kit.Notifier.Record_Changed (2, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if (not Item.Read_Only) and then Item.Created then
         ...
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T2_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T2_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T2_Name_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T2_Table_Index_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Table_Index, 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Database_Mutex.Shared_Unlock;
         Kit.Notifier.Record_Created (1, Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record));
         Kit.Notifier.Record_Created (2, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not Item.Read_Only then
         ...
         declare
            T2_Access : constant Kit_Record_Cache.Cache_Access :=
               Kit_Record_Cache.Get (Marlowe.Database_Index (Item.M_Index));
         begin
            T2_Access.Db := Item.T2_Data;
         end;
      end if;
      if not Item.Read_Only then
         Kit_Root_Record_Cache.Unlock (Marlowe.Database_Index
            (Item.T2_Data.Kit_Root_Record));
         Kit_Record_Cache.Unlock (Marlowe.Database_Index (Item.M_Index));
      end if;
      Item.M_Index := 0;
      Item.Created := False;
   end Finalize;
   ...
   begin
      ...
      Kit_Record_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
      ...
      Kit_Record_Impl.File_Mutex.Shared_Unlock;
      return Kit_Record_Reference (Db_Index);
   ...
   function Get (Ref : Kit_Record_Reference) return Kit_Record_Type is
   begin
      return Result : Kit_Record_Implementation do
         Kit_Record_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Record_Reference then
            Kit_Record_Cache.S_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T2_Data := Kit_Record_Cache.Get (Marlowe.Database_Index
               (Result.M_Index), False).Db;
            Kit_Root_Record_Cache.S_Lock (Marlowe.Database_Index
               (Result.T2_Data.Kit_Root_Record));
            Result.T1_Data := Kit_Root_Record_Cache.Get
               (Marlowe.Database_Index (Result.T2_Data.Kit_Root_Record),
               False).Db;
            Kit_Record_Cache.Unlock (Marlowe.Database_Index
               (Result.M_Index));
            Kit_Root_Record_Cache.Unlock (Marlowe.Database_Index
               (Result.T2_Data.Kit_Root_Record));
         end if;
   ...
   begin
      if Field = "" then
         ...
         return Item.Name;
      elsif Field = "table_index" then
         return Positive'Image (Item.Table_Index);
      elsif Field = "record_length" then
         return Natural'Image (Item.Record_Length);
      else
         raise Constraint_Error with
           "no such field";
      end if;
   ...
   function Get_By_Name (Name : String) return Kit_Record_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (2);
      Kit_Record_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_Name_Ref,
   ...
   begin
      ...
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_Table_Index_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Table_Index, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Table_Index, 4)
            & Marlowe.Key_Storage.To_Storage_Array
   ...
   function Get_From_Kit_Root_Record (Kit_Root_Record :
      Kit_Root_Record_Reference) return Kit_Record_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      ...
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
   ...
   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Record_Implementation) return Kit_Root_Record_Reference
   is (Item.T2_Data.Kit_Root_Record);

   ----------------
   -- Get_Update --
   ----------------

   function Get_Update (Ref : Kit_Record_Reference)
      return Kit_Record_Update is
   begin
      return Result : Kit_Record_Update_Implementation do
         Kit_Record_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Record_Reference then
            Kit_Record_Cache.U_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T2_Data := Kit_Record_Cache.Get (Marlowe.Database_Index
               (Result.M_Index), False).Db;
   ...
   overriding function Identity (Item : Kit_Record_Implementation)
      return String is
   begin
      return "kit_record" & Kit_Record_Reference'Image
         (Item.Get_Kit_Record_Reference);
   end Identity;
   ...
   begin
      return Get_By_Name (Name) /= Null_Kit_Record_Reference;
   end Is_Name;
   ...
   function Is_Table_Index (Table_Index : Integer) return Boolean is
   begin
      return Get_By_Table_Index (Table_Index) /= Null_Kit_Record_Reference;
   end Is_Table_Index;
   ...
   overriding function Name (Item : Kit_Record_Implementation)
      return String is
      Result : Kit.Strings.String_Type
        renames Item.T2_Data.Name;
   begin
   ...
   is
   begin
      Handle.Handler (Kit_Record_Reference (Changed_Record));
   ...
   overriding procedure Notify_Table_Change (Handle :
      Kit_Record_Table_Notify_Record) is
   begin
      Handle.Handler.all;
   end Notify_Table_Change;

   --------------------------
   -- On_Kit_Record_Change --
   --------------------------
   ...
   begin
      ...
      Kit.Notifier.Add_Record_Change_Handler
        (2,
         ...
         Rec);
   end On_Kit_Record_Change;

   ---------------------------
   -- On_Kit_Record_Created --
   ---------------------------

   procedure On_Kit_Record_Created (Handler : Kit_Record_Notify_Handler) is
      Rec : Kit_Record_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Create_Handler (2, Rec);
   end On_Kit_Record_Created;

   ---------------------------
   -- On_Kit_Record_Deleted --
   ---------------------------

   procedure On_Kit_Record_Deleted (Handler : Kit_Record_Notify_Handler) is
      Rec : Kit_Record_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Delete_Handler (2, Rec);
   end On_Kit_Record_Deleted;

   --------------------------------
   -- On_Kit_Record_Table_Change --
   --------------------------------

   procedure On_Kit_Record_Table_Change (Handler :
      Kit_Record_Table_Notify_Handler) is
      Rec : Kit_Record_Table_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Table_Change_Handler (2, Rec);
   end On_Kit_Record_Table_Change;
   ...
   overriding function Record_Length (Item : Kit_Record_Implementation)
      return Integer is
      Result : Integer
        renames Item.T2_Data.Record_Length;
   begin
      return Result;
   end Record_Length;
   ...
   begin
      ...
      declare
         ...
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_Name_Ref, First_Key,
            Last_Key, Marlowe.Closed, Marlowe.Closed, Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Record_Reference
   ...
   function Scan_By_Table_Index return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (2);
      declare
   ...
   begin
      ...
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Start_Table_Index, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Finish_Table_Index, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T2_Table_Index_Ref,
   ...
   end Select_Bounded_By_Table_Index;
   ...
   ---------------------------
   -- Select_By_Table_Index --
   ---------------------------

   function Select_By_Table_Index (Table_Index : Integer) return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (2);
      declare
   ...
   begin
      if Field = "" then
         ...
         Item.Set_Table_Index (Positive'Value (Value));
      elsif Field = "record_length" then
         Item.Set_Record_Length (Natural'Value (Value));
      else
         raise Constraint_Error with
           "no such field";
      end if;
   ...
   is
      Target : Integer
        renames Item.T2_Data.Table_Index;
   begin
      Database_Mutex.Shared_Lock;
      Item.X_Lock;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (2,
            Marlowe_Keys.T2_Table_Index_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Marlowe.Key_Storage.To_Storage_Array (Item.Table_Index, 4),
   ...
   overriding procedure X_Lock (Item : Kit_Record_Implementation) is
   begin
      Kit_Root_Record_Cache.X_Lock (Marlowe.Database_Index
         (Item.T2_Data.Kit_Root_Record));
      Kit_Record_Cache.X_Lock (Marlowe.Database_Index (Item.M_Index));
   end X_Lock;
