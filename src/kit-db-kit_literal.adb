with Ada.Finalization;
with System.Storage_Elements;
with Marlowe.Data_Stores;
with Marlowe.Key_Storage;
with Kit.Cache;
with Kit.Notifier;
with Kit.Strings;
with Kit.Db.Kit_Deferred_Keys;
with Kit.Db.Marlowe_Keys;
with Kit.Db.Kit_Root_Record_Cache;
with Kit.Db.Kit_Root_Record_Impl;
with Kit.Db.Kit_Literal_Impl;
with Kit.Db.Kit_Literal_Cache;

package body Kit.Db.Kit_Literal is

   type Kit_Literal_Implementation is
        limited new Ada.Finalization.Limited_Controlled
     and Kit_Literal_Interface with
      record
         Finished        : Boolean;
         Forward         : Boolean;
         Read_Only       : Boolean;
         Created         : Boolean;
         Deleted         : Boolean;
         Scanning        : Boolean;
         Has_Finish      : Boolean;
         Start_Closed    : Boolean;
         Finish_Closed   : Boolean;
         Using_Key       : Boolean;
         Using_Key_Value : Boolean;
         Key_Ref         : Marlowe.Data_Stores.Key_Reference;
         M_Index         : Kit_Literal_Reference;
         T1_Data         :
            Kit_Root_Record_Impl.Kit_Root_Record_Database_Record;
         T14_Data        : Kit_Literal_Impl.Kit_Literal_Database_Record;
      end record;
   overriding function Has_Element (Item : Kit_Literal_Implementation)
      return Boolean;
   overriding procedure X_Lock (Item : Kit_Literal_Implementation);
   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Literal_Implementation) return Kit_Root_Record_Reference;
   overriding function Get_Kit_Literal_Reference (Item :
      Kit_Literal_Implementation) return Kit_Literal_Reference;
   overriding function Top_Record (Item : Kit_Literal_Implementation)
      return Record_Type;
   overriding function Name (Item : Kit_Literal_Implementation)
      return String;
   overriding function Kit_Enumeration (Item : Kit_Literal_Implementation)
      return Kit_Enumeration_Reference;
   overriding function Value (Item : Kit_Literal_Implementation)
      return Integer;
   overriding function Identity (Item : Kit_Literal_Implementation)
      return String;
   overriding function Get
     (Item  : Kit_Literal_Implementation;
      Field : String)
   return String;
   type Selection_Access is access Selection;
   type Iterator is
        new Selection_Iterator_Interfaces.Reversible_Iterator with
      record
         Container : Selection_Access;
      end record;
   overriding function First (Object : Iterator) return Cursor;

   overriding function Last (Object : Iterator) return Cursor;

   overriding function Next
     (Object   : Iterator;
      Position : Cursor)
   return Cursor;
   overriding function Previous
     (Object   : Iterator;
      Position : Cursor)
   return Cursor;
   type Kit_Literal_Table_Notify_Record is
        new Kit.Notifier.Table_Notify_Interface with
      record
         Handler : Kit_Literal_Table_Notify_Handler;
      end record;
   overriding procedure Notify_Table_Change (Handle :
      Kit_Literal_Table_Notify_Record);
   type Kit_Literal_Notify_Record is
        new Kit.Notifier.Record_Notify_Interface with
      record
         Handler : Kit_Literal_Notify_Handler;
      end record;
   overriding procedure Notify_Record_Change
     (Handle         : Kit_Literal_Notify_Record;
      Changed_Record : Marlowe.Database_Index);
   type Kit_Literal_Update_Implementation is
        limited new Kit_Literal_Implementation
     and Kit_Literal_Update_Interface
     and Record_Update_Interface with null record;
   overriding procedure Initialize (Item :
      in out Kit_Literal_Update_Implementation);
   overriding procedure Finalize (Item :
      in out Kit_Literal_Update_Implementation);
   overriding procedure Delete (Item :
      in out Kit_Literal_Update_Implementation);
   overriding procedure Set
     (Item  : in out Kit_Literal_Update_Implementation;
      Field : String;
      Value : String);
   overriding procedure Set_Name
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : String);
   overriding procedure Set_Kit_Enumeration
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Kit_Enumeration_Reference);
   overriding procedure Set_Kit_Enumeration
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Kit.Db.Kit_Enumeration.Kit_Enumeration_Type);
   overriding procedure Set_Value
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Integer);

   ------------------------
   -- Constant_Reference --
   ------------------------

   function Constant_Reference
     (Container : aliased Selection;
      Position  : Cursor)
   return Constant_Reference_Type
   is
   begin
      return (Element => Container.Elements.Constant_Reference (Position
      .Current).Element);
   end Constant_Reference;

   ------------
   -- Create --
   ------------

   function Create
     (Name            : String;
      Kit_Enumeration : Kit_Enumeration_Reference;
      Value           : Integer)
   return Kit_Literal_Reference
   is
      Result : Kit_Literal_Update := Create;
   begin
      Result.Set_Name (Name);
      Result.Set_Kit_Enumeration (Kit_Enumeration);
      Result.Set_Value (Value);
      return Result.Get_Kit_Literal_Reference;
   end Create;

   ------------
   -- Create --
   ------------

   function Create return Kit_Literal_Update is
      T1_Access : Kit_Root_Record_Cache.Cache_Access;
      T14_Access : Kit_Literal_Cache.Cache_Access;
   begin
      return Result : Kit_Literal_Update_Implementation do
         Result.Finished := False;
         Result.Created := True;
         Result.Deleted := False;
         Result.Scanning := False;
         Result.Read_Only := False;
         Result.Using_Key := False;
         Result.M_Index := 0;
         Memory_Mutex.Lock;
         T1_Access := new Kit_Root_Record_Cache.Cache_Record;
         T1_Access.Db.Top_Record := R_Kit_Literal;
         T14_Access := new Kit_Literal_Cache.Cache_Record;
         Memory_Mutex.Unlock;
         Kit_Root_Record_Impl.File_Mutex.Lock;
         T14_Access.Db.Kit_Root_Record := Kit_Root_Record_Reference
            (Marlowe_Keys.Handle.Insert_Record (1));
         T1_Access.Initialise (1, Marlowe.Database_Index
            (T14_Access.Db.Kit_Root_Record));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T1_Access));
         Kit_Root_Record_Impl.Write (Marlowe.Database_Index
            (T14_Access.Db.Kit_Root_Record), T1_Access.Db);
         T1_Access.X_Lock;
         Kit_Root_Record_Impl.File_Mutex.Unlock;
         Kit_Literal_Impl.File_Mutex.Lock;
         Result.M_Index := Kit_Literal_Reference
            (Marlowe_Keys.Handle.Insert_Record (14));
         T14_Access.Initialise (14, Marlowe.Database_Index (Result.M_Index));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T14_Access));
         Kit_Literal_Impl.Write (Marlowe.Database_Index (Result.M_Index),
            T14_Access.Db);
         T14_Access.X_Lock;
         Kit_Literal_Impl.File_Mutex.Unlock;
         Result.T1_Data := T1_Access.Db;
         Result.T14_Data := T14_Access.Db;
         Result.Read_Only := False;
      end return;
   end Create;

   ------------
   -- Create --
   ------------

   procedure Create
     (Name            : String;
      Kit_Enumeration : Kit_Enumeration_Reference;
      Value           : Integer)
   is
      Result : Kit_Literal_Update := Create;
   begin
      Result.Set_Name (Name);
      Result.Set_Kit_Enumeration (Kit_Enumeration);
      Result.Set_Value (Value);
   end Create;

   ------------
   -- Delete --
   ------------

   overriding procedure Delete (Item :
      in out Kit_Literal_Update_Implementation) is
   begin
      Database_Mutex.Shared_Lock;
      Item.Deleted := True;
      Database_Mutex.Shared_Unlock;
   end Delete;

   -------------
   -- Element --
   -------------

   function Element (Item : Cursor) return Kit_Literal_Reference is
   begin
      return List_Of_References.Element (Item.Current);
   end Element;

   --------------
   -- Finalize --
   --------------

   overriding procedure Finalize (Item :
      in out Kit_Literal_Update_Implementation) is
      use type System.Storage_Elements.Storage_Array;
   begin
      if Item.M_Index = 0 then
         return;
      end if;
      if Item.Deleted then
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T1_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.Kit_Enumeration))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_Value_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Value, 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_Enum_Value_Ref,
            Kit_Literal_Impl.Enum_Value_To_Storage (Item.Kit_Enumeration,
            Item.Value)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T14_Enum_Name_Ref,
            Kit_Literal_Impl.Enum_Name_To_Storage (Item.Kit_Enumeration,
            Item.Name)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Item.T1_Data.Deleted := True;
         Item.T14_Data.Deleted := True;
         Kit.Notifier.Record_Deleted (1, Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record));
         Kit.Notifier.Record_Deleted (14, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not (Item.Read_Only or else Item.Created) or else Item.Deleted then
         Kit.Notifier.Record_Changed (1, Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record));
         Kit.Notifier.Record_Changed (14, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if (not Item.Read_Only) and then Item.Created then
         Database_Mutex.Shared_Lock;
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T1_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.Kit_Enumeration))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_Value_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Value, 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_Enum_Value_Ref,
            Kit_Literal_Impl.Enum_Value_To_Storage (Item.Kit_Enumeration,
            Item.Value)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T14_Enum_Name_Ref,
            Kit_Literal_Impl.Enum_Name_To_Storage (Item.Kit_Enumeration,
            Item.Name)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Database_Mutex.Shared_Unlock;
         Kit.Notifier.Record_Created (1, Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record));
         Kit.Notifier.Record_Created (14, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not Item.Read_Only then
         declare
            T1_Access : constant Kit_Root_Record_Cache.Cache_Access :=
               Kit_Root_Record_Cache.Get (Marlowe.Database_Index
               (Item.T14_Data.Kit_Root_Record));
         begin
            T1_Access.Db := Item.T1_Data;
         end;
         declare
            T14_Access : constant Kit_Literal_Cache.Cache_Access :=
               Kit_Literal_Cache.Get (Marlowe.Database_Index (Item.M_Index));
         begin
            T14_Access.Db := Item.T14_Data;
         end;
      end if;
      if not Item.Read_Only then
         Kit_Root_Record_Cache.Unlock (Marlowe.Database_Index
            (Item.T14_Data.Kit_Root_Record));
         Kit_Literal_Cache.Unlock (Marlowe.Database_Index (Item.M_Index));
      end if;
      Item.M_Index := 0;
      Item.Created := False;
   end Finalize;

   -----------
   -- First --
   -----------

   overriding function First (Object : Iterator) return Cursor is
   begin
      return (Current => Object.Container.Elements.First);
   end First;

   -----------
   -- First --
   -----------

   function First (Container : Selection) return Cursor is
   begin
      return (Current => Container.Elements.First);
   end First;

   ------------------------------
   -- First_By_Kit_Enumeration --
   ------------------------------

   function First_By_Kit_Enumeration (Kit_Enumeration :
      Kit_Enumeration_Reference) return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end First_By_Kit_Enumeration;

   -------------------------
   -- First_By_Top_Record --
   -------------------------

   function First_By_Top_Record (Top_Record : Record_Type)
      return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end First_By_Top_Record;

   --------------------
   -- First_By_Value --
   --------------------

   function First_By_Value (Value : Integer) return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Value_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end First_By_Value;

   ---------
   -- Get --
   ---------

   function Get (Ref : Kit_Literal_Reference) return Kit_Literal_Type is
   begin
      return Result : Kit_Literal_Implementation do
         Kit_Literal_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Literal_Reference then
            Kit_Literal_Cache.S_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T14_Data := Kit_Literal_Cache.Get (Marlowe.Database_Index
               (Result.M_Index), False).Db;
            Kit_Root_Record_Cache.S_Lock (Marlowe.Database_Index
               (Result.T14_Data.Kit_Root_Record));
            Result.T1_Data := Kit_Root_Record_Cache.Get
               (Marlowe.Database_Index (Result.T14_Data.Kit_Root_Record),
               False).Db;
            Kit_Literal_Cache.Unlock (Marlowe.Database_Index
               (Result.M_Index));
            Kit_Root_Record_Cache.Unlock (Marlowe.Database_Index
               (Result.T14_Data.Kit_Root_Record));
         end if;
         Result.Finished := False;
         Result.Using_Key_Value := False;
         Result.Scanning := False;
         Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      end return;
   end Get;

   ---------
   -- Get --
   ---------

   overriding function Get
     (Item  : Kit_Literal_Implementation;
      Field : String)
   return String
   is
   begin
      if Field = "" then
         raise Constraint_Error with
           "missing field name";
      elsif Field = "top_record" then
         return Record_Type'Image (Item.Top_Record);
      elsif Field = "kit_root_record" then
         raise Constraint_Error with
           "field Kit_Root_Record is not readable";
      elsif Field = "name" then
         return Item.Name;
      elsif Field = "kit_enumeration" then
         return Kit_Enumeration_Reference'Image (Item.Kit_Enumeration);
      elsif Field = "value" then
         return Natural'Image (Item.Value);
      else
         raise Constraint_Error with
           "no such field";
      end if;
   end Get;

   ----------------------
   -- Get_By_Enum_Name --
   ----------------------

   function Get_By_Enum_Name
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Name            : String)
   return Kit_Literal_Reference
   is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Name_Ref,
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration, Name)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration, Name)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Get_By_Enum_Name;

   -----------------------
   -- Get_By_Enum_Value --
   -----------------------

   function Get_By_Enum_Value
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Value           : Integer)
   return Kit_Literal_Reference
   is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Value_Ref,
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration, Value)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration, Value)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Get_By_Enum_Value;

   ------------------------------
   -- Get_From_Kit_Root_Record --
   ------------------------------

   function Get_From_Kit_Root_Record (Kit_Root_Record :
      Kit_Root_Record_Reference) return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Root_Record)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Root_Record)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Get_From_Kit_Root_Record;

   -------------------------------
   -- Get_Kit_Literal_Reference --
   -------------------------------

   overriding function Get_Kit_Literal_Reference (Item :
      Kit_Literal_Implementation) return Kit_Literal_Reference
   is (Item.M_Index);

   -----------------------------------
   -- Get_Kit_Root_Record_Reference --
   -----------------------------------

   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Literal_Implementation) return Kit_Root_Record_Reference
   is (Item.T14_Data.Kit_Root_Record);

   ----------------
   -- Get_Update --
   ----------------

   function Get_Update (Ref : Kit_Literal_Reference)
      return Kit_Literal_Update is
   begin
      return Result : Kit_Literal_Update_Implementation do
         Kit_Literal_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Literal_Reference then
            Kit_Literal_Cache.U_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T14_Data := Kit_Literal_Cache.Get (Marlowe.Database_Index
               (Result.M_Index), False).Db;
            Kit_Root_Record_Cache.U_Lock (Marlowe.Database_Index
               (Result.T14_Data.Kit_Root_Record));
            Result.T1_Data := Kit_Root_Record_Cache.Get
               (Marlowe.Database_Index (Result.T14_Data.Kit_Root_Record),
               False).Db;
         end if;
         Result.Finished := False;
         Result.Using_Key_Value := False;
         Result.Scanning := False;
         Result.Read_Only := False;
         Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      end return;
   end Get_Update;

   -----------------
   -- Has_Element --
   -----------------

   overriding function Has_Element (Item : Kit_Literal_Implementation)
      return Boolean is
   begin
      return Item.M_Index /= 0;
   end Has_Element;

   -----------------
   -- Has_Element --
   -----------------

   function Has_Element (Item : Cursor) return Boolean is
   begin
      return List_Of_References.Has_Element (Item.Current);
   end Has_Element;

   --------------
   -- Identity --
   --------------

   overriding function Identity (Item : Kit_Literal_Implementation)
      return String is
   begin
      return "kit_literal" & Kit_Literal_Reference'Image
         (Item.Get_Kit_Literal_Reference);
   end Identity;

   ----------------
   -- Initialize --
   ----------------

   overriding procedure Initialize (Item :
      in out Kit_Literal_Update_Implementation) is
   begin
      Item.Finished := True;
      Item.Forward := True;
      Item.Read_Only := True;
      Item.Created := False;
      Item.Deleted := False;
      Item.Scanning := False;
      Item.Has_Finish := False;
      Item.Using_Key := False;
      Item.M_Index := 0;
      Item.Read_Only := True;
   end Initialize;

   --------------
   -- Is_Empty --
   --------------

   function Is_Empty (Container : Selection) return Boolean is
   begin
      return Has_Element (Container.First);
   end Is_Empty;

   ------------------
   -- Is_Enum_Name --
   ------------------

   function Is_Enum_Name
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Name            : String)
   return Boolean
   is
   begin
      return Get_By_Enum_Name (Kit_Enumeration, Name)
         /= Null_Kit_Literal_Reference;
   end Is_Enum_Name;

   -------------------
   -- Is_Enum_Value --
   -------------------

   function Is_Enum_Value
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Value           : Integer)
   return Boolean
   is
   begin
      return Get_By_Enum_Value (Kit_Enumeration, Value)
         /= Null_Kit_Literal_Reference;
   end Is_Enum_Value;

   -------------
   -- Iterate --
   -------------

   function Iterate (Container : Selection)
      return Selection_Iterator_Interfaces.Reversible_Iterator'Class is
   begin
      return Result : Iterator do
         Result.Container := Container'Unrestricted_Access;
      end return;
   end Iterate;

   ---------------------
   -- Kit_Enumeration --
   ---------------------

   overriding function Kit_Enumeration (Item : Kit_Literal_Implementation)
      return Kit_Enumeration_Reference is
      Result : Kit_Enumeration_Reference
        renames Item.T14_Data.Kit_Enumeration;
   begin
      return Result;
   end Kit_Enumeration;

   ----------
   -- Last --
   ----------

   overriding function Last (Object : Iterator) return Cursor is
   begin
      return (Current => Object.Container.Elements.Last);
   end Last;

   ----------
   -- Last --
   ----------

   function Last (Container : Selection) return Cursor is
   begin
      return (Current => Container.Elements.Last);
   end Last;

   -----------------------------
   -- Last_By_Kit_Enumeration --
   -----------------------------

   function Last_By_Kit_Enumeration (Kit_Enumeration :
      Kit_Enumeration_Reference) return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Backward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Last_By_Kit_Enumeration;

   ------------------------
   -- Last_By_Top_Record --
   ------------------------

   function Last_By_Top_Record (Top_Record : Record_Type)
      return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Backward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Last_By_Top_Record;

   -------------------
   -- Last_By_Value --
   -------------------

   function Last_By_Value (Value : Integer) return Kit_Literal_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      Kit_Literal_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Value_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First),
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last), Marlowe.Closed, Marlowe.Closed,
            Marlowe.Backward);
      begin
         if M.Valid then
            Db_Index := Marlowe.Key_Storage.To_Database_Index (M.Get_Key);
         end if;
      end;
      Kit_Literal_Impl.File_Mutex.Shared_Unlock;
      return Kit_Literal_Reference (Db_Index);
   end Last_By_Value;

   ------------
   -- Length --
   ------------

   function Length (Container : Selection) return Natural is
      Result : Natural := 0;
      It : Cursor := Container.First;
   begin
      while Has_Element (It) loop
         Result := Result + 1;
         Next (It);
      end loop;
      return Result;
   end Length;

   ----------
   -- Name --
   ----------

   overriding function Name (Item : Kit_Literal_Implementation)
      return String is
      Result : Kit.Strings.String_Type
        renames Item.T14_Data.Name;
   begin
      return Result.Text (1 .. Result.Length);
   end Name;

   ----------
   -- Next --
   ----------

   procedure Next (Position : in out Cursor) is
   begin
      List_Of_References.Next (Position.Current);
   end Next;

   ----------
   -- Next --
   ----------

   overriding function Next
     (Object   : Iterator;
      Position : Cursor)
   return Cursor
   is
   begin
      return (Current => List_Of_References.Next (Position.Current));
   end Next;

   ----------
   -- Next --
   ----------

   function Next (Position : Cursor) return Cursor is
   begin
      return (Current => List_Of_References.Next (Position.Current));
   end Next;

   --------------------------
   -- Notify_Record_Change --
   --------------------------

   overriding procedure Notify_Record_Change
     (Handle         : Kit_Literal_Notify_Record;
      Changed_Record : Marlowe.Database_Index)
   is
   begin
      Handle.Handler (Kit_Literal_Reference (Changed_Record));
   end Notify_Record_Change;

   -------------------------
   -- Notify_Table_Change --
   -------------------------

   overriding procedure Notify_Table_Change (Handle :
      Kit_Literal_Table_Notify_Record) is
   begin
      Handle.Handler.all;
   end Notify_Table_Change;

   ---------------------------
   -- On_Kit_Literal_Change --
   ---------------------------

   procedure On_Kit_Literal_Change
     (Reference : Kit_Literal_Reference;
      Handler   : Kit_Literal_Notify_Handler)
   is
      Rec : Kit_Literal_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Change_Handler
        (14,
         Marlowe.Database_Index (Reference),
         Rec);
   end On_Kit_Literal_Change;

   ----------------------------
   -- On_Kit_Literal_Created --
   ----------------------------

   procedure On_Kit_Literal_Created (Handler : Kit_Literal_Notify_Handler) is
      Rec : Kit_Literal_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Create_Handler (14, Rec);
   end On_Kit_Literal_Created;

   ----------------------------
   -- On_Kit_Literal_Deleted --
   ----------------------------

   procedure On_Kit_Literal_Deleted (Handler : Kit_Literal_Notify_Handler) is
      Rec : Kit_Literal_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Delete_Handler (14, Rec);
   end On_Kit_Literal_Deleted;

   ---------------------------------
   -- On_Kit_Literal_Table_Change --
   ---------------------------------

   procedure On_Kit_Literal_Table_Change (Handler :
      Kit_Literal_Table_Notify_Handler) is
      Rec : Kit_Literal_Table_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Table_Change_Handler (14, Rec);
   end On_Kit_Literal_Table_Change;

   --------------
   -- Previous --
   --------------

   procedure Previous (Position : in out Cursor) is
   begin
      List_Of_References.Previous (Position.Current);
   end Previous;

   --------------
   -- Previous --
   --------------

   overriding function Previous
     (Object   : Iterator;
      Position : Cursor)
   return Cursor
   is
   begin
      return (Current => List_Of_References.Previous (Position.Current));
   end Previous;

   --------------
   -- Previous --
   --------------

   function Previous (Position : Cursor) return Cursor is
   begin
      return (Current => List_Of_References.Previous (Position.Current));
   end Previous;

   -----------------------
   -- Scan_By_Enum_Name --
   -----------------------

   function Scan_By_Enum_Name return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array (1 .. 80) := (others => 0);
         Last_Key : constant Storage_Array (1 .. 80) :=
            (others => System.Storage_Elements.Storage_Element'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Name_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Scan_By_Enum_Name;

   ------------------------
   -- Scan_By_Enum_Value --
   ------------------------

   function Scan_By_Enum_Value return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array (1 .. 20) := (others => 0);
         Last_Key : constant Storage_Array (1 .. 20) :=
            (others => System.Storage_Elements.Storage_Element'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Scan_By_Enum_Value;

   -----------------------------
   -- Scan_By_Kit_Enumeration --
   -----------------------------

   function Scan_By_Kit_Enumeration return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array (1 .. 16) := (others => 0);
         Last_Key : constant Storage_Array (1 .. 16) :=
            (others => System.Storage_Elements.Storage_Element'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Scan_By_Kit_Enumeration;

   ------------------------
   -- Scan_By_Top_Record --
   ------------------------

   function Scan_By_Top_Record return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array (1 .. 12) := (others => 0);
         Last_Key : constant Storage_Array (1 .. 12) :=
            (others => System.Storage_Elements.Storage_Element'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Top_Record_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Scan_By_Top_Record;

   -------------------
   -- Scan_By_Value --
   -------------------

   function Scan_By_Value return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array (1 .. 12) := (others => 0);
         Last_Key : constant Storage_Array (1 .. 12) :=
            (others => System.Storage_Elements.Storage_Element'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Scan_By_Value;

   ----------------------------------
   -- Select_Bounded_By_Top_Record --
   ----------------------------------

   function Select_Bounded_By_Top_Record
     (Start_Top_Record  : Record_Type;
      Finish_Top_Record : Record_Type)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Start_Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Finish_Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Top_Record_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_Bounded_By_Top_Record;

   -----------------------------
   -- Select_Bounded_By_Value --
   -----------------------------

   function Select_Bounded_By_Value
     (Start_Value  : Integer;
      Finish_Value : Integer)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Start_Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Finish_Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_Bounded_By_Value;

   -------------------------
   -- Select_By_Enum_Name --
   -------------------------

   function Select_By_Enum_Name
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Name            : String)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration, Name)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration, Name)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Name_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_By_Enum_Name;

   --------------------------
   -- Select_By_Enum_Value --
   --------------------------

   function Select_By_Enum_Value
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Value           : Integer)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration, Value)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration, Value)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_By_Enum_Value;

   -------------------------------
   -- Select_By_Kit_Enumeration --
   -------------------------------

   function Select_By_Kit_Enumeration (Kit_Enumeration :
      Kit_Enumeration_Reference) return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Kit_Enumeration)) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Kit_Enumeration_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_By_Kit_Enumeration;

   --------------------------
   -- Select_By_Top_Record --
   --------------------------

   function Select_By_Top_Record (Top_Record : Record_Type)
      return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Top_Record), 4) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Top_Record_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_By_Top_Record;

   ---------------------
   -- Select_By_Value --
   ---------------------

   function Select_By_Value (Value : Integer) return Selection is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Marlowe.Key_Storage.To_Storage_Array (Value, 4)
            & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_By_Value;

   --------------------------------------
   -- Select_Enum_Name_Bounded_By_Name --
   --------------------------------------

   function Select_Enum_Name_Bounded_By_Name
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Start_Name      : String;
      Finish_Name     : String)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration,
            Start_Name) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Name_To_Storage (Kit_Enumeration,
            Finish_Name) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Name_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_Enum_Name_Bounded_By_Name;

   ----------------------------------------
   -- Select_Enum_Value_Bounded_By_Value --
   ----------------------------------------

   function Select_Enum_Value_Bounded_By_Value
     (Kit_Enumeration : Kit_Enumeration_Reference;
      Start_Value     : Integer;
      Finish_Value    : Integer)
   return Selection
   is
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Deferred_Keys.Check_Keys (14);
      declare
         use System.Storage_Elements;
         First_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration,
            Start_Value) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'First);
         Last_Key : constant Storage_Array :=
            Kit_Literal_Impl.Enum_Value_To_Storage (Kit_Enumeration,
            Finish_Value) & Marlowe.Key_Storage.To_Storage_Array
            (Marlowe.Database_Index'Last);
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T14_Enum_Value_Ref,
            First_Key, Last_Key, Marlowe.Closed, Marlowe.Closed,
            Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Literal_Reference
                  (Marlowe.Key_Storage.To_Database_Index (Mark.Get_Key)));
               Mark.Next;
            end loop;
         end return;
      end;
   end Select_Enum_Value_Bounded_By_Value;

   ---------
   -- Set --
   ---------

   overriding procedure Set
     (Item  : in out Kit_Literal_Update_Implementation;
      Field : String;
      Value : String)
   is
   begin
      if Field = "" then
         raise Constraint_Error with
           "missing field name";
      elsif Field = "top_record" then
         raise Constraint_Error with
           "field Top_Record is not writable";
      elsif Field = "kit_root_record" then
         raise Constraint_Error with
           "field Kit_Root_Record is not writable";
      elsif Field = "name" then
         Item.Set_Name (Value);
      elsif Field = "kit_enumeration" then
         Item.Set_Kit_Enumeration (Kit_Enumeration_Reference'Value (Value));
      elsif Field = "value" then
         Item.Set_Value (Natural'Value (Value));
      else
         raise Constraint_Error with
           "no such field";
      end if;
   end Set;

   -------------------------
   -- Set_Kit_Enumeration --
   -------------------------

   overriding procedure Set_Kit_Enumeration
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Kit.Db.Kit_Enumeration.Kit_Enumeration_Type)
   is
   begin
      Item.Set_Kit_Enumeration (Value.Get_Kit_Enumeration_Reference);
   end Set_Kit_Enumeration;

   -------------------------
   -- Set_Kit_Enumeration --
   -------------------------

   overriding procedure Set_Kit_Enumeration
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Kit_Enumeration_Reference)
   is
      Target : Kit_Enumeration_Reference
        renames Item.T14_Data.Kit_Enumeration;
   begin
      Database_Mutex.Shared_Lock;
      Item.X_Lock;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Kit_Enumeration_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
               (Item.Kit_Enumeration)),
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
               (Value)));
      end if;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Enum_Value_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Kit_Literal_Impl.Enum_Value_To_Storage (Item.Kit_Enumeration,
               Item.Value),
            Kit_Literal_Impl.Enum_Value_To_Storage (Value, Item.Value));
      end if;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Enum_Name_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Kit_Literal_Impl.Enum_Name_To_Storage (Item.Kit_Enumeration,
               Item.Name),
            Kit_Literal_Impl.Enum_Name_To_Storage (Value, Item.Name));
      end if;
      Target := Value;
      Database_Mutex.Shared_Unlock;
   end Set_Kit_Enumeration;

   --------------
   -- Set_Name --
   --------------

   overriding procedure Set_Name
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : String)
   is
      Target : Kit.Strings.String_Type
        renames Item.T14_Data.Name;
   begin
      Database_Mutex.Shared_Lock;
      Item.X_Lock;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Enum_Name_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Kit_Literal_Impl.Enum_Name_To_Storage (Item.Kit_Enumeration,
               Item.Name),
            Kit_Literal_Impl.Enum_Name_To_Storage (Item.Kit_Enumeration,
               Value));
      end if;
      Target.Length := Value'Length;
      Target.Text (1 .. Value'Length) := Value;
      Database_Mutex.Shared_Unlock;
   end Set_Name;

   ---------------
   -- Set_Value --
   ---------------

   overriding procedure Set_Value
     (Item  : in out Kit_Literal_Update_Implementation;
      Value : Integer)
   is
      Target : Integer
        renames Item.T14_Data.Value;
   begin
      Database_Mutex.Shared_Lock;
      Item.X_Lock;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Value_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Marlowe.Key_Storage.To_Storage_Array (Item.Value, 4),
            Marlowe.Key_Storage.To_Storage_Array (Value, 4));
      end if;
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (14,
            Marlowe_Keys.T14_Enum_Value_Ref,
            Marlowe.Database_Index (Item.M_Index),
            Kit_Literal_Impl.Enum_Value_To_Storage (Item.Kit_Enumeration,
               Item.Value),
            Kit_Literal_Impl.Enum_Value_To_Storage (Item.Kit_Enumeration,
               Value));
      end if;
      Target := Value;
      Database_Mutex.Shared_Unlock;
   end Set_Value;

   ----------------
   -- Top_Record --
   ----------------

   overriding function Top_Record (Item : Kit_Literal_Implementation)
      return Record_Type is
      Result : Record_Type
        renames Item.T1_Data.Top_Record;
   begin
      return Result;
   end Top_Record;

   -----------
   -- Value --
   -----------

   overriding function Value (Item : Kit_Literal_Implementation)
      return Integer is
      Result : Integer
        renames Item.T14_Data.Value;
   begin
      return Result;
   end Value;

   ------------
   -- X_Lock --
   ------------

   overriding procedure X_Lock (Item : Kit_Literal_Implementation) is
   begin
      Kit_Root_Record_Cache.X_Lock (Marlowe.Database_Index
         (Item.T14_Data.Kit_Root_Record));
      Kit_Literal_Cache.X_Lock (Marlowe.Database_Index (Item.M_Index));
   end X_Lock;

end Kit.Db.Kit_Literal;
