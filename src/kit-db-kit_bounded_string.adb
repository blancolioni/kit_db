...
with Kit.Db.Kit_Bounded_String_Cache;

package body Kit.Db.Kit_Bounded_String is

   type Kit_Bounded_String_Implementation is
        limited new Ada.Finalization.Limited_Controlled
     and Kit_Bounded_String_Interface with
      record
         Finished        : Boolean;
         Forward         : Boolean;
         Read_Only       : Boolean;
         ...
         M_Index         : Kit_Bounded_String_Reference;
         T1_Data         :
   ...
   overriding function Has_Element (Item : Kit_Bounded_String_Implementation)
      return Boolean;
   overriding procedure X_Lock (Item : Kit_Bounded_String_Implementation);
   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Bounded_String_Implementation) return Kit_Root_Record_Reference;
   overriding function Get_Kit_Type_Reference (Item :
      Kit_Bounded_String_Implementation) return Kit_Type_Reference;
   overriding function Get_Kit_String_Reference (Item :
      Kit_Bounded_String_Implementation) return Kit_String_Reference;
   overriding function Get_Kit_Bounded_String_Reference (Item :
      Kit_Bounded_String_Implementation) return Kit_Bounded_String_Reference;
   overriding function Top_Record (Item : Kit_Bounded_String_Implementation)
      return Record_Type;
   overriding function Length (Item : Kit_Bounded_String_Implementation)
      return Integer;
   overriding function Size (Item : Kit_Bounded_String_Implementation)
      return Integer;
   overriding function Name (Item : Kit_Bounded_String_Implementation)
      return String;
   overriding function Identity (Item : Kit_Bounded_String_Implementation)
      return String;
   overriding function Get
     (Item  : Kit_Bounded_String_Implementation;
      Field : String)
   return String;
   ...
   overriding function Previous
     (Object   : Iterator;
      Position : Cursor)
   return Cursor;
   type Kit_Bounded_String_Table_Notify_Record is
        new Kit.Notifier.Table_Notify_Interface with
      record
         Handler : Kit_Bounded_String_Table_Notify_Handler;
      end record;
   overriding procedure Notify_Table_Change (Handle :
      Kit_Bounded_String_Table_Notify_Record);
   type Kit_Bounded_String_Notify_Record is
   ...
   overriding procedure Notify_Record_Change
     (Handle         : Kit_Bounded_String_Notify_Record;
      Changed_Record : Marlowe.Database_Index);
   type Kit_Bounded_String_Update_Implementation is
        limited new Kit_Bounded_String_Implementation
     and Kit_Bounded_String_Update_Interface
     and Record_Update_Interface with null record;
   overriding procedure Initialize (Item :
      in out Kit_Bounded_String_Update_Implementation);
   overriding procedure Finalize (Item :
      in out Kit_Bounded_String_Update_Implementation);
   overriding procedure Delete (Item :
      in out Kit_Bounded_String_Update_Implementation);
   overriding procedure Set
     (Item  : in out Kit_Bounded_String_Update_Implementation;
      Field : String;
      Value : String);
   ...
   function Create return Kit_Bounded_String_Update is
      ...
      T12_Access : Kit_Bounded_String_Cache.Cache_Access;
   begin
      return Result : Kit_Bounded_String_Update_Implementation do
         ...
         T1_Access.Db.Top_Record := R_Kit_Bounded_String;
         T4_Access := new Kit_Type_Cache.Cache_Record;
         T10_Access := new Kit_String_Cache.Cache_Record;
         T12_Access := new Kit_Bounded_String_Cache.Cache_Record;
         Memory_Mutex.Unlock;
         Kit_Root_Record_Impl.File_Mutex.Lock;
         T12_Access.Db.Kit_Root_Record := Kit_Root_Record_Reference
            (Marlowe_Keys.Handle.Insert_Record (1));
         T1_Access.Initialise (1, Marlowe.Database_Index
            (T12_Access.Db.Kit_Root_Record));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T1_Access));
         Kit_Root_Record_Impl.Write (Marlowe.Database_Index
            (T12_Access.Db.Kit_Root_Record), T1_Access.Db);
         T1_Access.X_Lock;
         Kit_Root_Record_Impl.File_Mutex.Unlock;
         Kit_Type_Impl.File_Mutex.Lock;
         T12_Access.Db.Kit_Type := Kit_Type_Reference
            (Marlowe_Keys.Handle.Insert_Record (4));
         T4_Access.Db.Kit_Root_Record := T12_Access.Db.Kit_Root_Record;
         T4_Access.Initialise (4, Marlowe.Database_Index
            (T12_Access.Db.Kit_Type));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T4_Access));
         Kit_Type_Impl.Write (Marlowe.Database_Index
            (T12_Access.Db.Kit_Type), T4_Access.Db);
         T4_Access.X_Lock;
         Kit_Type_Impl.File_Mutex.Unlock;
         Kit_String_Impl.File_Mutex.Lock;
         T12_Access.Db.Kit_String := Kit_String_Reference
            (Marlowe_Keys.Handle.Insert_Record (10));
         T10_Access.Db.Kit_Root_Record := T12_Access.Db.Kit_Root_Record;
         T10_Access.Db.Kit_Type := T12_Access.Db.Kit_Type;
         T10_Access.Initialise (10, Marlowe.Database_Index
            (T12_Access.Db.Kit_String));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T10_Access));
         Kit_String_Impl.Write (Marlowe.Database_Index
            (T12_Access.Db.Kit_String), T10_Access.Db);
         T10_Access.X_Lock;
         Kit_String_Impl.File_Mutex.Unlock;
         Kit_Bounded_String_Impl.File_Mutex.Lock;
         Result.M_Index := Kit_Bounded_String_Reference
            (Marlowe_Keys.Handle.Insert_Record (12));
         T12_Access.Initialise (12, Marlowe.Database_Index (Result.M_Index));
         Kit.Cache.Insert (Kit.Cache.Cache_Entry (T12_Access));
         Kit_Bounded_String_Impl.Write (Marlowe.Database_Index
            (Result.M_Index), T12_Access.Db);
         T12_Access.X_Lock;
         Kit_Bounded_String_Impl.File_Mutex.Unlock;
         Result.T1_Data := T1_Access.Db;
         Result.T4_Data := T4_Access.Db;
         Result.T10_Data := T10_Access.Db;
         Result.T12_Data := T12_Access.Db;
         Result.Read_Only := False;
      end return;
   end Create;
   ...
   is
      Result : Kit_Bounded_String_Update := Create;
   begin
      Result.Set_Length (Length);
      Result.Set_Size (Size);
      Result.Set_Name (Name);
      return Result.Get_Kit_Bounded_String_Reference;
   end Create;
   ...
   overriding procedure Delete (Item :
      in out Kit_Bounded_String_Update_Implementation) is
   begin
   ...
   function Element (Item : Cursor) return Kit_Bounded_String_Reference is
   begin
   ...
   begin
      ...
      if Item.Deleted then
         ...
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_String)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T12_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T12_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T12_Name_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T12_T4_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_Type))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Delete (Marlowe_Keys.T12_T10_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_String))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         ...
         Kit.Notifier.Record_Deleted (10, Marlowe.Database_Index
            (Item.T12_Data.Kit_String));
         Kit.Notifier.Record_Deleted (12, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not (Item.Read_Only or else Item.Created) or else Item.Deleted then
         ...
         Kit.Notifier.Record_Changed (10, Marlowe.Database_Index
            (Item.T12_Data.Kit_String));
         Kit.Notifier.Record_Changed (12, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if (not Item.Read_Only) and then Item.Created then
         ...
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_String)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T12_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
            (Item.Top_Record), 4)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T12_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_Root_Record))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T12_Name_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64)
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T12_T4_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_Type))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         Marlowe_Keys.Handle.Insert (Marlowe_Keys.T12_T10_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.T12_Data.Kit_String))
         & Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
            (Item.M_Index)));
         ...
         Kit.Notifier.Record_Created (10, Marlowe.Database_Index
            (Item.T12_Data.Kit_String));
         Kit.Notifier.Record_Created (12, Marlowe.Database_Index
            (Item.M_Index));
      end if;
      if not Item.Read_Only then
         ...
         declare
            T12_Access : constant Kit_Bounded_String_Cache.Cache_Access :=
               Kit_Bounded_String_Cache.Get (Marlowe.Database_Index
               (Item.M_Index));
         begin
            T12_Access.Db := Item.T12_Data;
         end;
      end if;
      if not Item.Read_Only then
         ...
         Kit_String_Cache.Unlock (Marlowe.Database_Index
            (Item.T12_Data.Kit_String));
         Kit_Bounded_String_Cache.Unlock (Marlowe.Database_Index
            (Item.M_Index));
      end if;
      Item.M_Index := 0;
      Item.Created := False;
   ...
   function First_By_Top_Record (Top_Record : Record_Type)
      return Kit_Bounded_String_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      Kit_Bounded_String_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_Top_Record_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Record_Type'Pos
      ...
      Kit_Bounded_String_Impl.File_Mutex.Shared_Unlock;
      return Kit_Bounded_String_Reference (Db_Index);
   ...
   function Get (Ref : Kit_Bounded_String_Reference)
      return Kit_Bounded_String_Type is
   begin
      return Result : Kit_Bounded_String_Implementation do
         Kit_Bounded_String_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Bounded_String_Reference then
            Kit_Bounded_String_Cache.S_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T12_Data := Kit_Bounded_String_Cache.Get
               (Marlowe.Database_Index (Result.M_Index), False).Db;
            Kit_String_Cache.S_Lock (Marlowe.Database_Index
               (Result.T12_Data.Kit_String));
            Result.T10_Data := Kit_String_Cache.Get (Marlowe.Database_Index
               (Result.T12_Data.Kit_String), False).Db;
            Kit_Type_Cache.S_Lock (Marlowe.Database_Index
               (Result.T12_Data.Kit_Type));
            Result.T4_Data := Kit_Type_Cache.Get (Marlowe.Database_Index
               (Result.T12_Data.Kit_Type), False).Db;
            Kit_Root_Record_Cache.S_Lock (Marlowe.Database_Index
               (Result.T12_Data.Kit_Root_Record));
            Result.T1_Data := Kit_Root_Record_Cache.Get
               (Marlowe.Database_Index (Result.T12_Data.Kit_Root_Record),
               False).Db;
            Kit_Bounded_String_Cache.Unlock (Marlowe.Database_Index
               (Result.M_Index));
            Kit_String_Cache.Unlock (Marlowe.Database_Index
   ...
   function Get_By_Name (Name : String)
      return Kit_Bounded_String_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      ...
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_Name_Ref,
   ...
   function Get_From_Kit_Root_Record (Kit_Root_Record :
      Kit_Root_Record_Reference) return Kit_Bounded_String_Reference is
      use type System.Storage_Elements.Storage_Array;
      Db_Index : Marlowe.Database_Index := 0;
   begin
      Kit_Deferred_Keys.Check_Keys (1);
      ...
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_T1_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
   ...
   begin
      ...
      Kit_Deferred_Keys.Check_Keys (12);
      Kit_Bounded_String_Impl.File_Mutex.Shared_Lock;
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_T10_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
   ...
   begin
      ...
      declare
         M : constant Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_T4_Idx_Ref,
            Marlowe.Key_Storage.To_Storage_Array (Marlowe.Database_Index
   ...
   --------------------------------------
   -- Get_Kit_Bounded_String_Reference --
   --------------------------------------
   ...
   overriding function Get_Kit_Root_Record_Reference (Item :
      Kit_Bounded_String_Implementation) return Kit_Root_Record_Reference
   is (Item.T12_Data.Kit_Root_Record);
   ...
   function Get_Update (Ref : Kit_Bounded_String_Reference)
      return Kit_Bounded_String_Update is
   begin
      return Result : Kit_Bounded_String_Update_Implementation do
         Kit_Bounded_String_Impl.File_Mutex.Shared_Lock;
         Result.M_Index := Ref;
         if Result.M_Index /= Null_Kit_Bounded_String_Reference then
            Kit_Bounded_String_Cache.U_Lock (Marlowe.Database_Index
               (Result.M_Index));
            Result.T12_Data := Kit_Bounded_String_Cache.Get
               (Marlowe.Database_Index (Result.M_Index), False).Db;
   ...
   overriding function Identity (Item : Kit_Bounded_String_Implementation)
      return String is
   begin
      return "kit_bounded_string" & Kit_Bounded_String_Reference'Image
         (Item.Get_Kit_Bounded_String_Reference);
   end Identity;
   ...
   is
   begin
      Handle.Handler (Kit_Bounded_String_Reference (Changed_Record));
   ...
   overriding procedure Notify_Table_Change (Handle :
      Kit_Bounded_String_Table_Notify_Record) is
   begin
      Handle.Handler.all;
   end Notify_Table_Change;

   ----------------------------------
   -- On_Kit_Bounded_String_Change --
   ----------------------------------
   ...
   procedure On_Kit_Bounded_String_Change
     (Reference : Kit_Bounded_String_Reference;
      Handler   : Kit_Bounded_String_Notify_Handler)
   is
      Rec : Kit_Bounded_String_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Change_Handler
        (12,
         ...
         Rec);
   end On_Kit_Bounded_String_Change;

   -----------------------------------
   -- On_Kit_Bounded_String_Created --
   -----------------------------------

   procedure On_Kit_Bounded_String_Created (Handler :
      Kit_Bounded_String_Notify_Handler) is
      Rec : Kit_Bounded_String_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Create_Handler (12, Rec);
   end On_Kit_Bounded_String_Created;

   -----------------------------------
   -- On_Kit_Bounded_String_Deleted --
   -----------------------------------
   ...
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Record_Delete_Handler (12, Rec);
   end On_Kit_Bounded_String_Deleted;

   ----------------------------------------
   -- On_Kit_Bounded_String_Table_Change --
   ----------------------------------------

   procedure On_Kit_Bounded_String_Table_Change (Handler :
      Kit_Bounded_String_Table_Notify_Handler) is
      Rec : Kit_Bounded_String_Table_Notify_Record;
   begin
      Rec.Handler := Handler;
      Kit.Notifier.Add_Table_Change_Handler (12, Rec);
   end On_Kit_Bounded_String_Table_Change;
   ...
   begin
      ...
      declare
         ...
         Mark : Marlowe.Data_Stores.Data_Store_Cursor :=
            Marlowe_Keys.Handle.Search (Marlowe_Keys.T12_Name_Ref, First_Key,
            Last_Key, Marlowe.Closed, Marlowe.Closed, Marlowe.Forward);
      begin
         return Result : Selection do
            while Mark.Valid loop
               Result.Elements.Append (Kit_Bounded_String_Reference
      ...
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (4,
            ...
            Marlowe.Database_Index (Item.T12_Data.Kit_Type),
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64),
      ...
      if not Item.Created then
         Kit_Deferred_Keys.Key_Changed
           (4,
            ...
            Marlowe.Database_Index (Item.T12_Data.Kit_String),
            Marlowe.Key_Storage.To_Storage_Array (Item.Name, 64),
            Marlowe.Key_Storage.To_Storage_Array (Value, 64));
      end if;
      if not Item.Created then
   ...
   overriding procedure X_Lock (Item : Kit_Bounded_String_Implementation) is
   begin
      ...
      Kit_String_Cache.X_Lock (Marlowe.Database_Index
         (Item.T12_Data.Kit_String));
      Kit_Bounded_String_Cache.X_Lock (Marlowe.Database_Index
         (Item.M_Index));
   end X_Lock;
