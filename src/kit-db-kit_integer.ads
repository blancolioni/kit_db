private with Ada.Containers.Doubly_Linked_Lists;
with Ada.Iterator_Interfaces;
with Kit.Db.Kit_Root_Record;
with Kit.Db.Kit_Type;

package Kit.Db.Kit_Integer is

   F_Top_Record : constant String := "top_record";
   F_Kit_Root_Record : constant String := "kit_root_record";
   F_Size : constant String := "size";
   F_Name : constant String := "name";
   F_Kit_Type : constant String := "kit_type";
   F_Low : constant String := "low";
   F_High : constant String := "high";

   type Kit_Integer_Interface is limited interface
     and Record_Interface
     and Search_Interface
     and Kit.Db.Kit_Root_Record.Kit_Root_Record_Interface
     and Kit.Db.Kit_Type.Kit_Type_Interface;

   subtype Kit_Integer_Type is Kit_Integer_Interface'Class;

   function Get_Kit_Integer_Reference (Item : Kit_Integer_Interface)
      return Kit_Integer_Reference
      is abstract;

   function Low (Item : Kit_Integer_Interface) return Integer
      is abstract;

   function High (Item : Kit_Integer_Interface) return Integer
      is abstract;

   type Kit_Integer_Update_Interface is limited interface
     and Kit_Integer_Interface
     and Kit.Db.Kit_Root_Record.Kit_Root_Record_Update_Interface
     and Kit.Db.Kit_Type.Kit_Type_Update_Interface;

   subtype Kit_Integer_Update is Kit_Integer_Update_Interface'Class;
   function Create return Kit_Integer_Update;
   procedure Create
     (Size : Integer;
      Name : String;
      Low  : Integer;
      High : Integer);
   function Create
     (Size : Integer;
      Name : String;
      Low  : Integer;
      High : Integer)
   return Kit_Integer_Reference;

   type Cursor is private;
   function Has_Element (Item : Cursor) return Boolean;
   type Constant_Reference_Type
      (Element : not null access constant Kit_Integer_Reference) is private
   with Implicit_Dereference => Element;
   type Reference_Type (Element : not null access Kit_Integer_Type) is
      private
   with Implicit_Dereference => Element;
   package Selection_Iterator_Interfaces is
     new Ada.Iterator_Interfaces
          (Cursor,
         Has_Element);
   type Selection is tagged private
   with Constant_Indexing => Constant_Reference,
        Default_Iterator => Iterate,
        Iterator_Element => Kit_Integer_Reference;
   function Iterate (Container : Selection)
      return Selection_Iterator_Interfaces.Reversible_Iterator'Class;
   function Constant_Reference
     (Container : aliased Selection;
      Position  : Cursor)
   return Constant_Reference_Type;
   function Is_Empty (Container : Selection) return Boolean;
   function Element (Item : Cursor) return Kit_Integer_Reference;
   function Length (Container : Selection) return Natural;
   function Scan_By_Top_Record return Selection;

   function First_By_Top_Record (Top_Record : Record_Type)
      return Kit_Integer_Reference;

   function Last_By_Top_Record (Top_Record : Record_Type)
      return Kit_Integer_Reference;

   function Select_By_Top_Record (Top_Record : Record_Type) return Selection;

   function Select_Bounded_By_Top_Record
     (Start_Top_Record  : Record_Type;
      Finish_Top_Record : Record_Type)
   return Selection;

   function Get_From_Kit_Root_Record (Kit_Root_Record :
      Kit_Root_Record_Reference) return Kit_Integer_Reference;

   function Scan_By_Name return Selection;

   function Get_By_Name (Name : String) return Kit_Integer_Reference;

   function Select_By_Name (Name : String) return Selection;

   function Select_Bounded_By_Name
     (Start_Name  : String;
      Finish_Name : String)
   return Selection;

   function Get_From_Kit_Type (Kit_Type : Kit_Type_Reference)
      return Kit_Integer_Reference;

   function Get (Ref : Kit_Integer_Reference) return Kit_Integer_Type;
   function Get_Update (Ref : Kit_Integer_Reference)
      return Kit_Integer_Update;

   function First (Container : Selection) return Cursor;
   function Last (Container : Selection) return Cursor;
   function Next (Position : Cursor) return Cursor;
   function Previous (Position : Cursor) return Cursor;
   procedure Next (Position : in out Cursor);
   procedure Previous (Position : in out Cursor);

   type Kit_Integer_Notify_Handler is access
     procedure (Reference : Kit_Integer_Reference);
   type Kit_Integer_Table_Notify_Handler is access
     procedure;

   procedure On_Kit_Integer_Table_Change (Handler :
      Kit_Integer_Table_Notify_Handler);
   procedure On_Kit_Integer_Change
     (Reference : Kit_Integer_Reference;
      Handler   : Kit_Integer_Notify_Handler);
   procedure On_Kit_Integer_Created (Handler : Kit_Integer_Notify_Handler);
   procedure On_Kit_Integer_Deleted (Handler : Kit_Integer_Notify_Handler);
   procedure Set_Low
     (Item  : in out Kit_Integer_Update_Interface;
      Value : Integer)
      is abstract;

   procedure Set_High
     (Item  : in out Kit_Integer_Update_Interface;
      Value : Integer)
      is abstract;

private

   package List_Of_References is
     new Ada.Containers.Doubly_Linked_Lists
          (Kit_Integer_Reference);
   type Cursor is
      record
         Current : List_Of_References.Cursor;
      end record;
   type Constant_Reference_Type
      (Element : not null access constant Kit_Integer_Reference) is
          null record;
   type Reference_Type (Element : not null access Kit_Integer_Type) is
          null record;
   type Selection is tagged
      record
         Elements : List_Of_References.List;
      end record;
   pragma Inline (Constant_Reference);

end Kit.Db.Kit_Integer;
